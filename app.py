...
def ask_sterling_brain(user_voice_prompt: str) -> str:
    ...
    # 1. Try Groq (Using verified baseline Llama model ID)
    if GROQ_API_KEY:
        try:
            url = "https://groq.com"
            headers = {
            ...
            data = {
                "model": "llama3-8b-8192",  # Swapped to ultra-stable baseline model for testing
                "messages": [
        ...
        diagnostic_info["groq_key"] = "Missing from Environment"

    # 2. Fallback to Gemini (Using explicit structural declaration configuration)
    if GEMINI_API_KEY:
        try:
            url = f"https://googleapis.com{GEMINI_API_KEY.strip()}"
        ...
        diagnostic_info["gemini_key"] = "Missing from Environment"
            
    return f"Diagnostics Triggered - Raw Pipeline Responses: {json.dumps(diagnostic_info)}"
